...
registerSuite('authoritzation tests', () => {
    ...
    return {
        ...
        'tests for authz can export': {
                        ...
                        can: [
                            {
                                ...
                                name: 'post:delete',
                                when: () => { throw new Error('Test error'); }
                            }
                        ],
                        inherits: ['guest'],
                    },
            ...
            'multi role check': () => {
                const permitted = can(['user', 'guest'], 'post:view');

                assert.isTrue(permitted);
            },

            'undefined role': () => {
                const permitted = can('admin', 'post:view');

                assert.isFalse(permitted);
            },

            'throwing permission': () => {
                const permitted = can('user', 'post:delete');
